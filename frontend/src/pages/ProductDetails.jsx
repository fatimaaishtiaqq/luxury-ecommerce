    ...
    if (data.images && Array.isArray(data.images) && data.images.length > 0) {
        ...
        if (typeof imgPath === 'string' && imgPath.trim() !== '') {
            if (imgPath.startsWith('http') || imgPath.startsWith('/images/')) {
                ...
                parsedImage = `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/uploads/${fileName}`;
            }
        }
    }
...
/** First image in product images array — used as default main image on load. No variation applied. */
        ...
        <div className="w-full mb-20 text-[var(--text-primary)]">
            ...
            <div className="px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
                    ...
                    {/* Left: Image block — main image fixed aspect 3/4; thumbnail carousel same height, scrolls inside (never stretches page) */}
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
